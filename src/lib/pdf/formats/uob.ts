	...
	dataToExtract.forEach((page, pageNum) => {
		page.blocks.forEach((block, blockIdx) => {
			...
			if (currentCard) {
				...
				const transactionDate = parseTxnDate(
					secondLine.text,
					extractedData.statementDate,
				);
...
const extractDataAccount: PdfFormatExtractor = (dataToExtract, userId) => {
	...
	const getAccountDetails = (firstPage: MuPdfStructuredTextPage) => {
		const depositHeaderIdx = firstPage.blocks.findIndex(
			(block) =>
	...
	const getStatementDate = (firstPage: MuPdfStructuredTextPage) => {
		const periodBlock = firstPage.blocks.find((block) =>
			block.lines.find((line) => line.text.includes("Statement of Account")),
		);
	...
	const getTransactionFromBlock = (
		block?: MuPdfStructuredTextBlock,
		nextBlock?: MuPdfStructuredTextBlock,
	): TransactionsInsertSchema | undefined => {
		if (!block) return;
		const firstLine = block.lines[0];
		if (!firstLine) return;
		...
		const splitOver2Blocks =
			firstLineIsDate && block.lines.at(-1)?.text !== "  ";
		...
		const transactionDate =
			parseDateString(`${firstLine.text} ${statementYear}`, "DD MMM YYYY") ||
			firstLine.text;
	...
	const remainingBlocks = dataToExtract.slice(1).flatMap((p) => p.blocks);
	if (!firstPage) {
	...
	Object.keys(extractedData.accounts).forEach((accountName) => {
		const accountHeaderIdx = remainingBlocks.findIndex((block) =>
			block.lines.find((l) => l.text.includes(accountName)),
		);
		...
		const transactionEndIdx = remainingBlocks.findIndex(
			(block, findingIdx) =>
				findingIdx > accountHeaderIdx &&
				block.lines.find((line) => line.text.includes("Total")),
		);

		for (let i = transactionsStartIdx; i < transactionEndIdx; i++) {
			const transactionBlock = remainingBlocks[i];
			const txn = getTransactionFromBlock(
				transactionBlock,
				remainingBlocks[i + 1],
			);
			if (txn) {
