...
const extractDataCard: PdfFormatExtractor = (dataToExtract, userId) => {
	...
	const statementCycleBlock = firstPage.blocks.find((block) =>
		block.lines.find((line) => line.text.includes("Statement cycle")),
	);
	...
	if (datesLine) {
		const matches = Array.from(
			datesLine.text.matchAll(/(\d{2} \w{3} )(\d{4})/g),
		);
	...
	const transactionStartIdx = secondPage.blocks.findIndex((block) =>
		block.lines.find((line) => line.text.includes("Previous balance")),
	);
	const transactionEndIdx = secondPage.blocks.findIndex((block) =>
		block.lines.find((line) => line.text.includes("Total outstanding balance")),
	);
	...
	for (let i = transactionStartIdx + 1; i < transactionEndIdx; i++) {
		...
		if (fullTransactionInBlock) {
			const description = transactionBlock.lines
				.slice(2, -1)
				.map((l) => l.text.trim())
				.join(" ")
				.trim();
			...
			const description =
				descriptionBlock?.lines
					.concat(amountBlock?.lines.slice(0, 2) || [])
					.map((l) => l.text.trim())
					.join(" ")
					.trim() || "";
