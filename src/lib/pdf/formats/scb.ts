...
const extractDataCard: PdfFormatExtractor = (dataToExtract, userId) => {
	...
	const cardDetailsBlockStartIdx = firstPage.blocks.findIndex((block) =>
		block.lines.find((line) => line.text.includes("Account/Card No.")),
	);
	const creditLimitBlock = firstPage.blocks.find((block) =>
		block.lines.find((line) => line.text.includes("Approved Credit Limit")),
	);
	const dueDateBlock = firstPage.blocks.find((block) =>
		block.lines.find((line) => line.text.includes("Payment Due Date")),
	);
	const statementDateBlock = firstPage.blocks.find((block) =>
		block.lines.find((line) => line.text.includes("Statement Date")),
	);

	for (
		let i = cardDetailsBlockStartIdx + 1;
		i < cardDetailsBlockStartIdx + 5;
		i++
	) {
	...
	if (creditLimitBlock) {
		...
		const creditLimitAmountBlock = firstPage.blocks.find((block) =>
			block.lines.find((line) => line.y === yCoord),
		);
		const creditLimit = creditLimitAmountBlock?.lines[1]?.text;
		if (creditLimit) {
			try {
				extractedData.creditLimit = parseFloat(
					creditLimit.trim().replaceAll(",", ""),
				);
			} catch (error) {
	...
	Object.keys(extractedData.cards).forEach((cardName) => {
		const startTransactionIdx = allBlocks.findIndex((block) =>
			block.lines.find((line) =>
				line.text.includes("BALANCE FROM PREVIOUS STATEMENT"),
			),
		);
		...
		for (let i = startTransactionIdx + 1; i < endTransactionIdx; i++) {
			...
			if (unformattedAmount) {
				try {
					amount =
						parseFloat(unformattedAmount.trim().replaceAll(",", "")) * -1;
				} catch (error) {
			...
			const descriptionBlock = allBlocks.find((block) =>
				block.lines.find((line) => line.y === rowYCoord),
			);
			let description = "";
			if (descriptionBlock) {
