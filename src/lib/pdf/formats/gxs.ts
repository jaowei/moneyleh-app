...
const getStatementDetails = (allText: string[][]) => {
	const accountInfoBlock = allText.find(
		(block) =>
			block.length === 3 && parseDateString(block[0] || "", "D MMM YYYY"),
	);
	if (!accountInfoBlock) {
		throw ParsingErrors.statementDate;
	}
...
const getAccountTransactions = (
	...
	for (const [idx, block] of allLines.entries()) {
		...
		const isTxnFooter =
			block.length === 3 && block[0]?.text.match(/-?[\d,]+\.\d+/);
	...
	Object.entries(accountsInfo).forEach(([accountName, idxs]) => {
		...
		const transactions = groupedLines.map((t) => {
			...
			const description = t
				.slice(2, -2)
				.map((l) => (typeof l !== "string" ? l.text : l))
				.join(" ");
...
const extractDataAccount: PdfFormatExtractor = (pages, userId) => {
	...
	const allLines = pages.flatMap((page) =>
		page.blocks.map((block) => block.lines),
	);
	...
	data.accounts = getAccountTransactions(
		allLines,
		data.statementDate,
		userId,
		accountNumber,
