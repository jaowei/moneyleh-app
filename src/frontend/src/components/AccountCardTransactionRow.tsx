...
import { backendRouteClient, type GetTransactionDataRes } from "../lib/backend-clients";
...
const getUpdater = (property: keyof Transaction, transaction: Transaction, valueToUpdate: string) => {
    switch (property) {
...
export const AccountCardTransactionRow = ({ transaction, transactionIndex, onTagEditorOpen,
    setTransactions, userId }: TransactionRowProps) => {
    ...
    return (
        <tr>
            <EditableCell editing={editRow}
                value={date.toLocaleDateString()} onChange={(val) => handleCellChange(val, 'transactionDate')} />
            <EditableCell editing={editRow}
                value={transaction.description} onChange={(val) => handleCellChange(val, 'description')} />
            <td>{transaction.currency}</td>
            <EditableCell editing={editRow}
                value={transaction.amount} onChange={(val) => handleCellChange(val, 'amount')} />
            <td><TagTableViewer selectedTags={transaction.tags} canEdit={editRow} onTagChange={handleTagChange} /></td>
            <td><TagPicker onTagPickerClick={handleTagPickerClick} disabled={!editRow} /></td>
            <td>{
                editRow ? (<button className="btn btn-xs btn-secondary" onClick={handleSaveRowClick}>save</button>) : (
